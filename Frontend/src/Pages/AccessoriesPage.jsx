...
function AccessoriesPage() {
    ...
    return (
        <>
            <Navbar />
            <Box sx={{ height: "25rem", width: '100%', backgroundImage: "url('Images/accessorize.jpg')", backgroundRepeat: 'no-repeat', backgroundSize: 'cover', position: 'relative', imageResolution: 'from-image 300dpi' }}>
                <Typography varient="h6" component="div" sx={{ position: 'absolute', transform: 'translate(-50%,-50%)', top: '50%', left: '50%', textAlign: 'center' }}>
                    <Stack>
                        <List sx={{ fontSize: "2rem", fontWeight: 900, color: 'white' }}> Accessories Corner</List>
            ...
            <Container>
                <Box sx={{ py: 4 }}>
                    <Typography variant="h5" component="div" sx={{ fontWeight: 900 }}>Accessories Latest</Typography>
                ...
                <Box sx={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', alignItems: 'center', gap: '2rem', my: '1rem' }}>
                    {data.map((item, index) => (
                        <>
                            {(item.category === 'smartphones' || item.category === 'mens-watches' || item.category === 'mens-watches') ?
